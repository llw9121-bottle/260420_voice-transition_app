...
- `api_settings_dialog.py` - API Key 配置对话框，支持首次启动引导
...
3. **paragraphs** - 整理为自然段落
   - 默认：基于时间戳停顿分割，快速不消耗 Token
   - 可选：启用 **LLM 语义分段**，使用百炼大模型按语义重新整理，段落结构更自然
4. **behavior_match** - 关键行为匹配，使用 LLM 识别用户定义的行为模式
   - 逐步处理：清洗文本 → LLM语义段落整理 → 行为匹配
   - 支持超长文本自动分块，避免上下文溢出
   - 原位标记匹配结果，文档末尾自动生成**行为频率统计表格**（按置信度分类）
