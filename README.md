...
- **内存优化**：长时间录音内存使用稳定
- **暂停/继续录音**：支持暂停录音，之后继续录制同一个文档
- **自动保存临时文件**：录音过程中自动保存，防止应用崩溃数据丢失，启动时可恢复
- **可配置高级选项**：段落整理、自动分块均可开关配置
- **文本搜索与导航**：支持关键词搜索，高亮所有匹配项，上下导航快速定位
- **滚动锁定**：可锁定自动滚动，方便录音过程中查看历史内容
- **外观主题切换**：支持 System（跟随系统）、Light（浅色）、Dark（深色）三种主题，记住用户选择
- **行为配置增强**：
  - 内置 4 个常用场景预置模板（会议分析、面试分析、心理咨询、需求沟通），一键加载
  - 支持配置导入/导出 JSON 文件，方便备份和多设备同步
- **默认导出路径记忆**：记住用户上次选择的导出目录，下次导出默认使用
- **多语言ASR支持**：支持中文、粤语、英文、日语、韩语、德语、法语、西班牙语、俄语等多种识别语言，满足不同场景需求
- **VAD参数可配置**：可调节静音检测时长（200ms-2000ms），控制断句灵敏度：
  - 短时长（200-500ms）：灵敏断句，适合对话访谈场景
  - 长时长（1000-2000ms）：稳定断句，适合朗读演讲场景
- **中英双语LLM提示词**：根据识别语言自动选择对应语言提示词，行为匹配和段落整理效果更优
- **预置行为模板增强**：内置中英文模板，支持领导力评估等场景
- **用户体验增强**：
  - 实时录音时长显示，掌握录音进度
  - 录音音量可视化，直观判断麦克风输入（灵敏度优化）
  - 红色闪烁录音状态指示灯，一目了然
...
1. **选择识别语言**：在设置面板选择ASR识别语言（默认中文）
2. **调整VAD参数**：根据场景调整静音检测阈值（对话场景 400ms，朗读场景 1000ms+）
3. **选择格式化风格**：在界面右侧设置面板选择转录风格
4. **（可选）段落模式**：如果选择 `paragraphs`，可勾选"启用 LLM 语义分段"获得更自然的段落结构（消耗额外 Token）
5. **开始录音**：点击「开始录音」按钮，允许应用访问麦克风
6. **实时查看结果**：转录结果会实时显示在主界面，状态栏显示录音时长和音量
7. **停止录音**：点击「停止录音」结束转录
8. **导出文档**：点击「导出」按钮，选择导出格式保存文件

### 快捷键支持

| 快捷键 | 功能 |
|--------|------|
| `空格` | **智能切换**：未开始 → 开始，正在录音 → 暂停，暂停中 → 恢复 |
| `Esc` | 直接停止录音 |
| `Ctrl+S` | 导出文档 |
| `Ctrl+B` | 打开行为配置 |
...
**预置模板：**
应用内置多个常用场景模板，一键加载：
- 会议分析（中文）：识别需求提出、问题反馈、风险识别、决策安排
- 求职面试（中文）：识别项目经验、技能自评、求职意向、薪资要求
- 心理咨询（中文）：识别情绪表达、问题描述、改变意愿、资源探索
- 需求沟通（中文）：识别用户需求、问题反馈、确认验收、行动计划
- 领导力评估（英文）：识别Strategic Thinking、Achievement Orientation、Critical Analysis、Leadership Drive
